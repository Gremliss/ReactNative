import AsyncStorage from "@react-native-async-storage/async-storage";
import { useEffect, useState } from "react";
import {
  Alert,
  FlatList,
  Keyboard,
  StatusBar,
  StyleSheet,
  Text,
  TouchableWithoutFeedback,
  View,
} from "react-native";
import Note from "../components/Note";
import NoteInputModal from "../components/NoteInputModal";
import NoteMenuModal from "../components/NoteMenuModal";
import NotFound from "../components/NotFound";
import RoundIconBtn from "../components/RoundIconButton";
import SearchBar from "../components/SearchBar";
import { useNotes } from "../contexts/NoteProvider";
import colors from "../misc/colors";

const reverseData = (data) => {
  return data.sort((a, b) => {
    const aInt = parseInt(a.id);
    const bInt = parseInt(b.id);
    if (aInt < bInt) return 1;
    if (aInt == bInt) return 0;
    if (aInt > bInt) return -1;
  });
};

const NoteScreen = ({ user, navigation }) => {
  const [greet, setGreet] = useState("");
  const [modalVisible, setModalVisible] = useState(false);
  const [modalMenuVisible, setModalMenuVisible] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [resultNotFound, setResultNotFound] = useState(false);
  // const [notes, setNotes] = useState([]);
  const { notes, setNotes, findNotes } = useNotes();

  const findGreet = () => {
    const hrs = new Date().getHours();
    if (hrs === 0 || hrs < 12) return setGreet("Morning");
    if (hrs === 1 || hrs < 17) return setGreet("Afternoon");
    setGreet("Evening");
  };

  useEffect(() => {
    findGreet();
  });

  const reversedNotes = reverseData(notes);

  const handleOnSubmit = async (title, desc) => {
    const note = { id: Date.now(), title, desc, time: Date.now() };
    const updatedNotes = [...notes, note];
    setNotes(updatedNotes);
    await AsyncStorage.setItem("notes", JSON.stringify(updatedNotes));
  };

  const handleMenuOnSubmit = async (name, color) => {
    user.name = name;
    user.color = color;
    await AsyncStorage.setItem("user", JSON.stringify(user));
  };

  const openNote = async (note, user) => {
    navigation.navigate("NoteDetail", { note, user });
  };

  const displayDeleteAlert = (item) => {
    Alert.alert(
      "Are you sure?",
      "This will delete the note permanently",
      [{ text: "Delete", onPress: () => deleteNote(item) }, { text: "No" }],
      { cancelable: true }
    );
  };

  const deleteNote = async (item) => {
    const newNotes = notes.filter((n) => n.id !== item.id);
    setNotes(newNotes);
    await AsyncStorage.setItem("notes", JSON.stringify(newNotes));
  };

  const handleOnSearchInput = async (text) => {
    setSearchQuery(text);
    if (!text.trim()) {
      setSearchQuery("");
      setResultNotFound(false);
      return await findNotes();
    }
    const filteredNotes = notes.filter((note) => {
      if (note.title.toLowerCase().includes(text.toLowerCase())) {
        return note;
      }
    });

    if (filteredNotes.length) {
      setNotes([...filteredNotes]);
    } else {
      setResultNotFound(true);
    }
  };

  const handleOnClear = async () => {
    setSearchQuery("");
    setResultNotFound(false);
    await findNotes();
  };

  return (
    <>
      <StatusBar barStyle="dark-content" backgroundColor={colors.PRIMARY} />
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.container}>
          <Text style={styles.header}>
            Good {greet} {user.name}
          </Text>
          {notes.length ? (
            <SearchBar
              value={searchQuery}
              onChangeText={handleOnSearchInput}
              containerStyle={{ marginVertical: 15 }}
              onClear={handleOnClear}
              user={user}
            />
          ) : null}

          {resultNotFound ? (
            <NotFound />
          ) : (
            <FlatList
              data={reversedNotes}
              numColumns={2}
              columnWrapperStyle={{
                justifyContent: "space-between",
                marginBottom: 15,
              }}
              keyExtractor={(item) => item.id.toString()}
              renderItem={({ item }) => (
                <Note
                  onPress={() => openNote(item, user)}
                  onLongPress={() => displayDeleteAlert(item)}
                  item={item}
                  user={user}
                />
              )}
            />
          )}

          {!notes.length ? (
            <View
              style={[
                StyleSheet.absoluteFillObject,
                styles.emptyHeaderContainer,
              ]}
            >
              <Text style={styles.emptyHeader}>Add Notes</Text>
            </View>
          ) : null}
        </View>
      </TouchableWithoutFeedback>
      <RoundIconBtn
        onPress={() => setModalVisible(true)}
        antIconName={"plus"}
        style={styles.addBtn}
        user={user}
      />
      <RoundIconBtn
        onPress={() => setModalMenuVisible(true)}
        antIconName={"menufold"}
        style={styles.menuBtn}
        user={user}
      />
      <NoteInputModal
        visible={modalVisible}
        onClose={() => setModalVisible(false)}
        onSubmit={handleOnSubmit}
        user={user}
      />
      <NoteMenuModal
        visible={modalMenuVisible}
        onClose={() => setModalMenuVisible(false)}
        onSubmit={handleMenuOnSubmit}
        name={user.name}
        user={user}
      />
    </>
  );
};

const styles = StyleSheet.create({
  container: {
    paddingHorizontal: 20,
    flex: 1,
    zIndex: 1,
  },
  header: {
    fontSize: 25,
    fontWeight: "bold",
    marginTop: 10,
  },
  emptyHeader: {
    fontSize: 30,
    textTransform: "uppercase",
    fontWeight: "bold",
    opacity: 0.2,
    textAlign: "center",
  },
  emptyHeaderContainer: {
    flex: 1,
    justifyContent: "center",
    alignContent: "center",
    zIndex: -1,
  },
  addBtn: {
    position: "absolute",
    right: 15,
    bottom: 50,
    zIndex: 1,
  },
  menuBtn: {
    position: "absolute",
    left: 15,
    bottom: 50,
    zIndex: 1,
  },
});

export default NoteScreen;
